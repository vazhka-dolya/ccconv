...
mainw = Tk()
mainw.geometry("500x530")
mainw.title("ccconv")

mainw.config()

#Text Area Code
TextAreaInput = Text (mainw, height = 22, width = 18, relief = SUNKEN, bd = 2)
TextAreaOutput = Text (mainw, height = 22, width = 18, relief = SUNKEN, bd = 2)
CodeLabel1 = Label(mainw, text="Input Code:")
CodeLabel2 = Label(mainw, text="Output Code:")
    
CodeLabel1.config(font = ("Calibri", 10))
CodeLabel2.config(font = ("Calibri", 10))
    
CodeLabel1.place(x=20, y=15)
CodeLabel2.place(x=330, y=15)

TextAreaInput.place(x=20, y=40)
TextAreaOutput.place(x=330, y=40)

#Code Region
#"A" stands for "Addresses"
...
DropI = OptionMenu(mainw, DropInput, *CodeRegionOptions)
DropO = OptionMenu(mainw, DropOutput, *CodeRegionOptions)
DropI.place(x=20, y=435)
DropO.place(x=330, y=435)

#Region Label
def RegionLabel(LabelName, x, y):
    LabelName = Label(mainw, text="Region:")
    
    LabelName.config(font = ("Calibri", 10))
...
def Convert():
    ...
    for item in ChosenListInput:
        InputText = InputText.replace(item, ChosenListOutput[count])
        OutputText = InputText
        count += 1
    TextAreaOutput.delete("1.0", END)
    TextAreaOutput.insert("1.0", OutputText)

def ConvertButton(x, y):
    ConvertButton = Button(mainw,
            text="Convert",
            font = ("Calibri", 16),
            command = Convert)
    ConvertButton.place(x=x, y=y)

def AboutLabel(anchor):
    AboutLabel = Label(mainw,
            text = "ccconv v1.0.0\nMade by vdolya\nMy GitHub: https://github.com/vazhka-dolya")
    AboutLabel.place(relx = 0.5,
            rely = 1.0,
            anchor = anchor)

RegionLabel("Label1", 20, 415)
RegionLabel("Label2", 330, 415)
